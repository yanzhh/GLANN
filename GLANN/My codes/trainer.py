...
class Trainer(object):
    ...
    def __init__(self,
        ...
        # --- optimizer ---
        self.global_step = tf.contrib.framework.get_or_create_global_step(graph=None)              # 生成全局统一的训练步数
        self.learning_rate = config.learning_rate
        if config.lr_weight_decay:                                                                 # 设置指数衰减的lr
    ...
    def run_single_step_GLO(self, batch, dataset, step=None, is_train=True):
        _start_time = time.time()

        batch_chunk = self.session.run(batch)                                     # 创建 batch_train
        ...
        z_update_norm = z_update / norm[:, np.newaxis]                               # project updated z into unit ball
    ...
    def run_test_GLO(self, batch, is_train=False, repeat_times=8):
    ...
    def log_step_message_GLO(self, step, loss, loss_g_update,
...
if __name__ == '__main__':
    ...
    m, l = dataset_train.get_data(dataset_train.ids[0])                        # m: image size of length 3, l: label size of length 1
