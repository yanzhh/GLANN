...
def conv_out_size_same(size, stride):                      # output size of convolution when PADDING='SAME'
  return int(np.ceil(float(size) / float(stride)))         # np.ceil: 向上取整
...
def generator(z, batch_size, output_height, output_width, output_c_dim,
  ...
  with tf.variable_scope("generator", reuse=reuse):
    s_h, s_w = output_height, output_width                                            # 最终的output size, assume s_h=64, s_w=64.
    s_h2, s_w2 = conv_out_size_same(s_h, 2), conv_out_size_same(s_w, 2)               # 上一级output size, 32, 32
    ...
    s_h16, s_w16 = conv_out_size_same(s_h8, 2), conv_out_size_same(s_w8, 2)           # ..., 4, 4
                                                                                      # assume s_h=64, s_w=64.
    net = linear(z, gf_dim * 8 *s_h16 * s_w16, scope="g_fc1")                         # output size: gf_dim*8*s_h16*s_w16
